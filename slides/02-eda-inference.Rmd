...
## Always look at the data before {.smaller}
...
r <- diag(cor(anscombe)[5:8, 1:4])
names(r) <- paste("x", 1:4, "-y", 1:4, sep = "")
print(ascii(as.data.frame(r), digits = 4, align = "c"), type = "pandoc")
```



```{r anscombe, echo=FALSE, fig.width = 4.5, fig.height = 4.5}
data(anscombe)
...
```{r combn, echo=FALSE, results='asis', warning=FALSE}
m <- data.frame(t(combn(c(121, 118, 110, 34, 12, 22), 3)))
m$s <- apply(m[,1:3], 1, sum)
print(ascii(m[c(1:3, 18:20),]), type = "pandoc")
```

# Parametric approach to null hypothesis testing
